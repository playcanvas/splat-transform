...
const getReader = (type: string): ValueReader => {
    switch (type) {
        ...
        default:
            throw new Error(`readPly: unsupported ply type '${type}'`);
    }
};
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        switch (words[0]) {
            ...
            case 'element':
                ...
                break;
            case 'property':
                if (!current || words.length !== 3 || !getDataType(words[1]))
                    throw new Error('readPly: invalid property line');
...
const decodePlyToDataTable = async (source: ReadSource): Promise<DataTable> => {
    ...
    for (const element of headerElements) {
        ...
        const dataTable = await decodeElement(stream, element.properties, element.count, (r) =>
            bar.update(priorRows + r)
        );
        prior += element.count;
...
const readCompressedChunked = (source: ReadSource, header: PlyHeader, pool: ChunkDataPool): ChunkSource => {
    ...
    const shBands: SHBands =
        restCount === 0
            ? 0
            : ({ [SH_REST_COUNTS[1]]: 1, [SH_REST_COUNTS[2]]: 2, [SH_REST_COUNTS[3]]: 3 } as Record<number, SHBands>)[
                  restCount
              ];
    ...
    const read = async (request: ReadRequest): Promise<void> => {
        ...
        const minX = pos ? bcol('min_x') : null,
            minY = pos ? bcol('min_y') : null,
            minZ = pos ? bcol('min_z') : null;
        const maxX = pos ? bcol('max_x') : null,
            maxY = pos ? bcol('max_y') : null,
            maxZ = pos ? bcol('max_z') : null;
        const minSX = geo ? bcol('min_scale_x') : null,
            minSY = geo ? bcol('min_scale_y') : null,
            minSZ = geo ? bcol('min_scale_z') : null;
        const maxSX = geo ? bcol('max_scale_x') : null,
            maxSY = geo ? bcol('max_scale_y') : null,
            maxSZ = geo ? bcol('max_scale_z') : null;
        const sw = 3 + restCount;
        const hasColors = col ? bnd.hasColumn('min_r') : false;
        const minR = hasColors ? bcol('min_r') : null,
            minG = hasColors ? bcol('min_g') : null,
            minB = hasColors ? bcol('min_b') : null;
        const maxR = hasColors ? bcol('max_r') : null,
            maxG = hasColors ? bcol('max_g') : null,
            maxB = hasColors ? bcol('max_b') : null;
        ...
        if ('indices' in request) {
            ...
            for (const run of gatherRuns(count, (t) => rowAt(t) * vertexRowSize, vertexRowSize, costBytes)) {
                ...
                if (
                    (await readExact(
                        source.read(
                            vertexOffset + firstRow * vertexRowSize,
                            vertexOffset + (firstRow + rc) * vertexRowSize
                        ),
                        vBytes,
                        0,
                        vNeed
                ...
                if (wantSh) {
                    ...
                    if (
                        (await readExact(
                            source.read(shOffset + firstRow * shRowSize, shOffset + (firstRow + rc) * shRowSize),
                            sBytes,
                            0,
                            sNeed
                        )) !== sNeed
        ...
        if (
            (await readExact(
                source.read(vertexOffset + start * vertexRowSize, vertexOffset + (start + count) * vertexRowSize),
                vBytes,
                0,
                vBytes.length
            )) !== vBytes.length
        ...
        if (col && shEl) {
            ...
            if (
                (await readExact(
                    source.read(shOffset + start * shRowSize, shOffset + (start + count) * shRowSize),
                    sBytes,
                    0,
                    sBytes.length
                )) !== sBytes.length
...
const readPly = async (source: ReadSource, pool: ChunkDataPool): Promise<ChunkSource> => {
    ...
    if (header.format === 'binary_little_endian' && source.size !== undefined) {
        ...
        if (source.size !== expected) {
            throw new Error(
                `readPly: file size ${source.size} does not match header-implied size ${expected} (truncated or corrupt PLY)`
            );
        }
    }
    ...
    const fieldNames = (layer: ChunkLayer): string[] => {
        switch (layer) {
            case 'position':
                return ['x', 'y', 'z'];
            case 'geometric':
                return geometricCols;
            case 'color':
                return [...COLOR_DC_COLS, ...Array.from({ length: SH_REST_COUNTS[shBands] }, (_, k) => `f_rest_${k}`)];
            case 'other':
                return extras.map((e) => e.name);
        }
    };
    ...
    const read = async (request: ReadRequest): Promise<void> => {
        ...
        const requested: ChunkLayer[] = (['position', 'geometric', 'color', 'other'] as ChunkLayer[]).filter(
            (layer) => request[layer]
        );
