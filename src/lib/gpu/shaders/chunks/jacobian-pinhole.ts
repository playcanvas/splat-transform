...
const jacobianPinhole = /* wgsl */`
    let cov2 = cov2dPinhole(cx, cy, cz, invZ, uniforms.focalX, uniforms.focalY, c00, c01, c02, c11, c12, c22);
    var cov00 = cov2.x;
    var cov01 = cov2.y;
    var cov11 = cov2.z;
`;
 ...
 * shutter-close pose with that pose's focal lengths. Include once at
 * module scope, before `jacobianPinhole`.
 */
const jacobianPinholeFns = /* wgsl */`
fn cov2dPinhole(
    cx: f32, cy: f32, cz: f32, invZ: f32, focalX: f32, focalY: f32,
    c00: f32, c01: f32, c02: f32, c11: f32, c12: f32, c22: f32
) -> vec3<f32> {
    let limX = JACOBIAN_LIMIT_FACTOR * (f32(uniforms.imageWidth) * 0.5) / focalX;
    let limY = JACOBIAN_LIMIT_FACTOR * (f32(uniforms.imageHeight) * 0.5) / focalY;
    let txtz = clamp(cx * invZ, -limX, limX);
    let tytz = clamp(cy * invZ, -limY, limY);
    let jcx = txtz * cz;
    let jcy = tytz * cz;
    let jx0 = focalX * invZ;
    let jx2 = -focalX * jcx * invZ * invZ;
    let jy1 = focalY * invZ;
    let jy2 = -focalY * jcy * invZ * invZ;
