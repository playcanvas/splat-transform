...
import { allocatePlanPrefixes, storeBlockPlan } from './block-allocation';
import type { StoredBlockPlan } from './block-allocation';
            ...
            if (generation === 1 && N >= COHERENCE_MIN_N) {
                ...
                if (median > INCOHERENT_RUNS_PER_BLOCK) {
                    if (blocks.length > 1) {
                        ...
                        const filename = join(spill.scratchDir, `.decimate-stage-${Date.now().toString(36)}.tmp.ply`);
